...
import (
	...
	"github.com/JoaNMiFTW/go-movies-backend/internal/models"
)
...
func (m *PostgresDBRepo) UpdateMovieGenres(id int, genreIDs []int) error {
	...
	for _, n := range genreIDs {
		insertQuery := `INSERT INTO movies_genres (movie_id, genre_id) VALUES ($1, $2)`
		_, err := m.DB.ExecContext(ctx, insertQuery, id, n)
		if err != nil {
			return err
		}
	}
